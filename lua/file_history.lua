...
local FileHistory = {
  ...
  backup_file = function(self, dirname, filename)
    ...
    local proc =
      self:_git_command({ 'diff-index', '--quiet', 'HEAD', '--', backuppath })
  ...
  set_tag = function(self, tag) self.tag = tag end,
}
...
M.file_history = function(opts)
  local proc =
    FileHistory:file_history(vim.fn.expand('%:p:h'), vim.fn.expand('%:t'))
  return proc.stdout
end

M.set_tag = function(tag) FileHistory:set_tag(tag) end
...
M.purge_file = function(filename) local proc = FileHistory:purge_file(filename) end
